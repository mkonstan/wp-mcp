...
- **What it does not do.** It cannot write, and so it cannot switch itself on, mint a token, or change an option. That is a real difference from code editing, which is arbitrary code execution. The risk here is disclosure: total disclosure of everything in the database, and - on a server that has not pinned `secure_file_priv` or denied `FILE` - of whatever else MySQL can read off the disk.
